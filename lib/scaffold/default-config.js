...
/**
 * Will return the path and default bitcore-node-mue configuration. It will search for the
 * configuration file in the "~/.muecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.muecore');
  var defaultConfigFile = path.resolve(defaultPath, './bitcore-node-mue.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['bitcoind', 'web', 'insight-api-mue', 'insight-ui-mue'];
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, defaultPath, './data/mued')
          }
          "connect": [{
        "rpchost": "127.0.0.1",
        "rpcport": 29947,
