...
/**
 * Will return the path and bitcore-node-mue configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'bitcore-node-mue.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'bitcore-node-mue.json'))
  };
}
